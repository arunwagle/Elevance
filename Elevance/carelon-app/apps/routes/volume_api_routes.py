...
def get_permitted_folders():
    ...
    if is_admin:
        ...
        try:
            ...
            # Folders to hide from target selection (templates are managed separately)
            _HIDDEN_FOLDERS = {'/templates'}
            folders = []
            for item in result.get('items', []):
                if item.get('is_directory'):
                    name = item.get('name', '')
                    # Filter out template folders
                    if f'/{name}' in _HIDDEN_FOLDERS:
                        continue
                    folders.append({
                        'path': item['path'],
                        'display_name': name,
    ...
    _TEMPLATES_PATH = f'{volume_root}/templates'
    folders = []
    for row in rows:
